...
<head>
    ...
    <title>Tesla Products | AIX Investment Group</title>
    <style>
        :root {
            --aix-black: #070707;
            --aix-dark: #141414;
...
<body>
    <header class="properties-header">
        ...
        <a href="{{ route('tesla') }}">Tesla</a>
    </header>

    <section class="properties-hero">
        <div class="properties-wrap">
            <span class="eyebrow">Tesla</span>
            <h1>Tesla Products</h1>
            <p class="hero-text">Browse available Tesla products including cars and more.</p>
        </div>
    </section>

    <main class="properties-main">
        <div class="properties-wrap">
            @forelse ($teslaProducts as $teslaProduct)
                @if ($loop->first)
                    <div class="properties-grid">
                @endif

                <article class="property-card">
                    @if (! empty($teslaProduct->product_images))
                        <div class="property-image-slider" data-property-slider>
                            @foreach ($teslaProduct->product_images as $image)
                                <button class="property-image-trigger property-slide {{ $loop->first ? 'is-active' : '' }}" type="button" data-property-image="{{ $image }}" data-property-title="{{ $teslaProduct->title }}">
                                    <img src="{{ $image }}" alt="{{ $teslaProduct->title }}">
                                </button>
                            @endforeach
                        </div>
                    @else
                    ...
                    <div class="property-card-content">
                        <h2>{{ $teslaProduct->title }}</h2>
                        <p>{{ $teslaProduct->description }}</p>
                        <span class="property-price">${{ number_format($teslaProduct->price, 2) }}</span>
                        <div class="property-card-actions">
                            <a class="property-action" href="{{ route('tesla.payment', $teslaProduct) }}">Invest</a>
                            <a class="property-action secondary" href="{{ route('tesla.show', $teslaProduct) }}">View Details</a>
                        </div>
                    </div>
                </article>
            ...
            @empty
                <div class="empty-state">
                    No Tesla products are available yet. Add products from the admin panel to display them here.
                </div>
            @endforelse
        </div>
    </main>
